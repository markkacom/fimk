...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.checkBlockHeight = function(blockHeight) {
		...
		if (!NRS.dgsBlockPassed) {
			if ((!NRS.isTestNet && (NRS.lastBlockHeight >= NRS.DIGITAL_GOODS_STORE_BLOCK || (NRS.downloadingBlockchain && NRS.state.lastBlockchainFeederHeight >= NRS.DIGITAL_GOODS_STORE_BLOCK))) || (NRS.isTestNet && NRS.lastBlockHeight >= NRS.DIGITAL_GOODS_STORE_BLOCK_TEST)) {
				NRS.dgsBlockPassed = true;
				$(".dgs_block").not(".advanced, .optional_message, .optional_note").show();
			}
		}
		if (!NRS.PKAnnouncementBlockPassed) {
			if ((!NRS.isTestNet && (NRS.lastBlockHeight >= NRS.PUBLIC_KEY_ANNOUNCEMENT_BLOCK || (NRS.downloadingBlockchain && NRS.state.lastBlockchainFeederHeight >= NRS.PUBLIC_KEY_ANNOUNCEMENT_BLOCK))) || (NRS.isTestNet && NRS.lastBlockHeight >= NRS.PUBLIC_KEY_ANNOUNCEMENT_BLOCK_TEST)) {
	...
	NRS.incoming.updateDashboardBlocks = function(newBlocks) {
      ...
      /* XXX - Hardcoded the 200 FIM POS reward for now */
      /* XXX - TODO Replace hardcoded 200 with dynamic value */
			rows += "<tr><td><a href='#' data-block='" + String(block.height).escapeHTML() + "' data-blockid='" + String(block.block).escapeHTML() + "' class='block'" + (block.numberOfTransactions > 0 ? " style='font-weight:bold'" : "") + ">" + String(block.height).escapeHTML() + "</a></td><td data-timestamp='" + String(block.timestamp).escapeHTML() + "'>" + NRS.formatTimestamp(block.timestamp) + "</td><td>" + NRS.formatAmount(block.totalAmountNQT) + " + " + NRS.formatAmount(block.totalFeeNQT) + "</td><td><strong>"+ (parseFloat(NRS.formatAmount(block.totalFeeNQT)) + 200)  +"</strong></td><td>" + NRS.formatAmount(block.numberOfTransactions) + "</td></tr>";
		}
	...
	NRS.blocksPageLoaded = function(blocks) {
      ...
      /* XXX - Add POS reward to blocks table */
      /* XXX - TODO POS reward is hardcoded for now */
			rows += "<tr><td><a href='#' data-block='" + String(block.height).escapeHTML() + "' data-blockid='" + String(block.block).escapeHTML() + "' class='block'" + (block.numberOfTransactions > 0 ? " style='font-weight:bold'" : "") + ">" + String(block.height).escapeHTML() + "</a></td><td>" + NRS.formatTimestamp(block.timestamp) + "</td><td>" + NRS.formatAmount(block.totalAmountNQT) + "</td><td>" + NRS.formatAmount(block.totalFeeNQT) + " + <strong>200</strong></td><td>" + NRS.formatAmount(block.numberOfTransactions) + "</td><td>" + (block.generator != NRS.genesis ? "<a href='#' data-user='" + NRS.getAccountFormatted(block, "generator") + "' class='user_info'>" + NRS.getAccountTitle(block, "generator") + "</a>" : $.t("genesis")) + "</td><td>" + NRS.formatVolume(block.payloadLength) + "</td><td>" + Math.round(block.baseTarget / NRS.initialBaseTarget * 100).pad(4) + " %</td></tr>";
		}
