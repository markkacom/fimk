...
var NRS = (function(NRS, $, undefined) {
		...
		if (alias) {
			...
			NRS.sendRequest("getAlias", {
				...
				if (response.errorCode) {
					...
					} else if ((aliasURI = /acct:(.*)@fim/.exec(response.aliasURI)) || (aliasURI = /nacc:(.*)/.exec(response.aliasURI))) {
	...
	NRS.forms.setAlias = function($modal) {
		...
		if (data.type == "account") {
			if (!(/acct:(.*)@fim/.test(data.aliasURI)) && !(/nacc:(.*)/.test(data.aliasURI))) {
				if (/^(FIM\-)/i.test(data.aliasURI)) {
	...
	function setAliasType(type, uri) {
		...
		if (type == "uri") {
			...
			$("#register_alias_uri").val("").mask("FIM-****-****-****-*****");

			if (uri) {
				var match = uri.match(/acct:(.*)@fim/i);
				if (!match) {
				...
				} else if (!/^FIM\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(uri)) {
					uri = NRS.accountRS;
				}
