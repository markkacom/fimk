...
final class TransactionImpl implements Transaction {
    ...
    public byte[] getBytes() {
        try {
            ...
            buffer.putLong(type.hasRecipient() ? Convert.nullToZero(recipientId) : Genesis.CREATOR_ID); /* XXX - prevent transaction replay */
    ...
    static TransactionImpl parseTransaction(byte[] bytes) throws NxtException.ValidationException {
        try {
            ...
            Long recipientId = Convert.zeroToNull(buffer.getLong());   /* XXX - prevent transaction replay */
